...
let Message = class Message extends Component {
    ...
    setSeverity() {
        let classList$$1 = [SUCCESS, WARNING, INFO, ERROR];
    ...
    setVariant() {
        let classList$$1 = [FILLED, OUTLINED];
        removeClass([this.element], classList$$1);
     ...
     */
    destroy() {
        let cssClass = isNullOrUndefined(this.cssClass) ? [''] : this.cssClass.split(' ');
        let className = [SUCCESS, WARNING, INFO, ERROR, RTL$1, HIDE, OUTLINED, FILLED];
        let classList$$1 = (cssClass.length === 1 && cssClass[0] === '') ? className : className.concat(cssClass);
        removeClass([this.element], classList$$1);
...
let Skeleton = class Skeleton extends Component {
    ...
    destroy() {
        super.destroy();
        let attrs = ['role', 'aria-live', 'aria-busy', 'aria-label'];
        let cssClass = [];
        if (this.cssClass)
    ...
    updateShape() {
        let shapeCss = cssClassName[this.shape.toUpperCase() + 'SHAPE'];
        let removeCss = this.element.classList.value.match(/(e-skeleton-[^\s]+)/g);
        this.updateDimension();
        if (removeCss)
    ...
    updateDimension() {
        let width = (!this.width && (['Text', 'Rectangle'].indexOf(this.shape) > -1)) ? '100%' : formatUnit(this.width);
        let height = ['Circle', 'Square'].indexOf(this.shape) > -1 ? width : formatUnit(this.height);
        this.element.style.width = width;
        this.element.style.height = height;
    }
    updateEffect() {
        let removeCss = this.element.classList.value.match(/(e-shimmer-[^\s]+)/g);
        if (removeCss)
            removeClass([this.element], removeCss);
...
export { ToastPosition, ButtonModelProps, ToastAnimations, ToastAnimationSettings, Toast, ToastUtility, Severity, Variant, Message, Skeleton };
